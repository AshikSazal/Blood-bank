...
<body>
  ...
  <?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      $dup = mysqli_query($conn,"select * from user_biodata where email = '$email'");
      if(mysqli_num_rows($dup)==1){
        $emailErr = "Email is already use";
      }

      if(mysqli_num_rows($dup)==0 && !empty($_POST["fname"]) && !empty($_POST["lname"]) && !empty($_POST['year']) && !empty($_POST['month']) && !empty($_POST['date']) && !empty($_POST["blood_group"]) && !empty($_POST["email"]) && !empty($_POST['phone']) && !empty($_POST["postcode"]) && !empty($_POST["village"]) && !empty($_POST["district"]) && !empty($_POST["division"]) && !empty($_POST["cpassword"]) && !empty($_POST["password"]) && ($_POST['password']==$_POST['cpassword'])){
        $sql = "insert into request_user_biodata(fname,lname,dob,blood_group,phone,postcode,village,district,division,email,password)values('$fname','$lname','$dob','$blood_group','$phone','$postcode','$village','$district','$division','$email','$password')";
    ...
    <div class="row">
      ...
      <div class="col-lg-6 input-form">
        <form method="post" action="<?php echo $_SERVER['PHP_SELF'];?>">
          ...
          <input class="edpc" type="password" name="password" placeholder="Password"><br>
          <span> <?php echo $passwordErr;?></span><br>

          <input class="edpc" type="password" name="cpassword" placeholder="Confirm password"><br>
          <span> <?php echo $cpasswordErr;?></span><br>
          <button class="btn btn-dark" name='submit' value='sub' style="border-radius: 80px;">Create an account</button>
        </form>
      </div>
      <div class="col-lg-3"></div>
    </div>
